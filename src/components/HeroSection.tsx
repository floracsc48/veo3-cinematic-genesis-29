...
const HeroSection: React.FC<HeroSectionProps> = ({
  ...
  return <section ref={ref} className="relative min-h-screen flex items-center justify-center overflow-hidden bg-black">
      {/* Video Background */}
      <video ref={videoRef} className="absolute top-0 left-0 w-full h-full object-cover z-0" autoPlay muted loop playsInline onLoadedData={handleVideoLoad} onError={handleVideoError} onCanPlay={handleVideoLoad} style={{
      ...
      {videoLoaded && <button onClick={toggleVideoPlayback} className="absolute bottom-8 right-8 z-20 glass p-3 rounded-full hover:bg-white/20 transition-all duration-300 opacity-70 hover:opacity-100" title={isVideoPlaying ? "Pause video" : "Play video"}>
          {isVideoPlaying ? <Pause size={20} strokeWidth={1} className="text-white" /> : <Play size={20} strokeWidth={1} className="text-white" />}
        </button>}
      ...
      <div className={`relative z-10 text-center px-4 max-w-4xl mx-auto animate-in ${isIntersecting ? 'visible' : ''}`}>
        ...
        <div className="flex flex-col items-center space-y-4">
          {!hasAccess ? <>
              {!showInviteInput ? <button onClick={handleGetAccess} className="neuro-button px-8 py-4 font-light text-lg tracking-wide hover:animate-glow text-gray-950">
                  Get Early Access
                </button> : <form onSubmit={handleInviteSubmit} className="glass-card p-6 w-full max-w-md">
                  <div className="mb-4">
              ...
              <button onClick={handleDownload} className="neuro-button px-8 py-4 text-white font-light text-lg tracking-wide hover:animate-glow">
                Download Veo 3 App
              </button>
              
              <div className="relative">
                <button onClick={handleCopyPassword} className="glass px-6 py-2 text-white/70 hover:text-white transition-colors duration-300 font-light text-sm tracking-wide flex items-center space-x-2 group" title={copied ? "Copied!" : "Copy Archive Password"}>
                  {copied ? <>
