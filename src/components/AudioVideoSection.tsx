...
const AudioVideoSection: React.FC = () => {
  ...
  return (
    <section 
      id="audio-video" 
      ref={ref}
      className={`py-32 px-4 transition-all duration-1000 ${isIntersecting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
    >
      <div className="max-w-4xl mx-auto">
        <div 
          className={`text-center mb-16 transition-all duration-1000 delay-300 ${isIntersecting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
        >
        ...
        <div className={`transition-all duration-1000 delay-500 ${isIntersecting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="relative aspect-video bg-black rounded-2xl overflow-hidden mb-8">
            ...
            <button
              ...
              className="absolute left-4 top-1/2 transform -translate-y-1/2 button-3d p-3 rounded-full"
            >
            ...
            <button
              onClick={handleNext}
              className="absolute right-4 top-1/2 transform -translate-y-1/2 button-3d p-3 rounded-full"
            >
          ...
          <div className="mb-8">
            ...
            <div className="flex justify-between mt-2 text-white/50 text-sm">
              {videos.map((_, index) => (
                <span key={index} className={index === currentVideoIndex ? 'text-orange-400' : ''}>
                  {index + 1}
                </span>
              ))}
            </div>
          ...
          <div className="glass-card p-6 rounded-xl">
            <div className="flex items-start justify-between">
              ...
              {currentVideo.prompt.length > 120 && (
                <button
                  onClick={handlePromptExpand}
                  className="button-3d p-2 flex-shrink-0"
                >
